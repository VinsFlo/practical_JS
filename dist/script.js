  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      postData('assets/server.php', formData).then(function (res) {
        ...
        clearInputs();
        setTimeout(function () {
          statusMessage.remove(); //удаляем сообщение из формы
        }, 5000);
      });
    });
  });
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        document.body.classList.add('modal-open'); // 'modal-open' is the class from bootstrap
      });
    });
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    var target = e.target; // Проверяем, есть ли таргет, и удостоверяемся, дейтсвительно ли 
    // пользователь кликнул в определенный таб
